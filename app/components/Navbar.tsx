...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-gray-200/50 transition-all duration-300">
        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="flex items-center">
            <Link href="/" className="flex-shrink-0 flex items-center gap-2 group">
              ...
              {/* UPDATED: Added 'font-rotura' and increased text size slightly */}
          ...
          <div className="hidden md:flex items-center space-x-2">
            ...
            <Link 
              href="/landed-cost"
              className="px-4 py-2 text-sm font-medium text-white bg-gray-900 rounded-full hover:bg-gray-800 transition-colors shadow-lg shadow-gray-900/20 ml-4"
            >
              Get Started
            </Link>
      ...
      {/* --- MOBILE MENU --- */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 max-h-[80vh] overflow-y-auto shadow-xl">
          <div className="px-4 pt-2 pb-6 space-y-4">
            ...
            <MobileSection title="Listing Tools">
              ...
              <MobileLink href="/keywords">Keywords</MobileLink>
            ...
            <MobileSection title="Operations">
              ...
              <MobileLink href="/cbm-calculator">Shipping</MobileLink>
            </MobileSection>
          </div>
        </div>
      )}
...
function Dropdown({ title, icon, children }: { title: string, icon: React.ReactNode, children: React.ReactNode }) {
  return (
    <div className="relative group h-full flex items-center px-1">
      <button className="flex items-center space-x-1 text-sm font-medium text-gray-600 group-hover:text-blue-600 transition-colors py-2">
        <span>{title}</span>
      ...
      {/* Mega Menu Dropdown */}
      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top -translate-y-2 group-hover:translate-y-0 z-50">
        <div className="bg-white rounded-xl shadow-xl border border-gray-100/50 overflow-hidden ring-1 ring-black/5">
           {children}
        </div>
      </div>
    </div>
...
function MobileSection({ title, children }: { title: string, children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <h3 className="font-bold text-gray-900 text-sm border-b pb-1 mb-2">{title}</h3>
